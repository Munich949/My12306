...
package com.dlnu.index12306.biz.payservice.dao.algorithm;
...
public class PayDataBaseComplexAlgorithm implements ComplexKeysShardingAlgorithm {
    ...
    public Collection<String> doSharding(Collection availableTargetNames, ComplexKeysShardingValue shardingValue) {
        ...
        if (CollUtil.isNotEmpty(columnNameAndShardingValuesMap)) {
            ...
            Collection<Comparable<Long>> customerOrderSnCollection = columnNameAndShardingValuesMap.get(orderSn);
            if (CollUtil.isNotEmpty(customerOrderSnCollection)) {
                String dbSuffix;
                Comparable<?> comparable = customerOrderSnCollection.stream().findFirst().get();
                ...
                String dbSuffix;
                String paySn = "pay_sn";
                Collection<Comparable<Long>> paySnCollection = columnNameAndShardingValuesMap.get(paySn);
                Comparable<?> comparable = paySnCollection.stream().findFirst().get();
                if (comparable instanceof String) {
                    String actualPaySn = comparable.toString();
                    dbSuffix = String.valueOf(hashShardingValue(actualPaySn.substring(Math.max(actualPaySn.length() - 6, 0))) % shardingCount / tableShardingCount);
                } else {
                    dbSuffix = String.valueOf(hashShardingValue((Long) comparable % 1000000) % shardingCount / tableShardingCount);
                }
                result.add("ds_" + dbSuffix);
