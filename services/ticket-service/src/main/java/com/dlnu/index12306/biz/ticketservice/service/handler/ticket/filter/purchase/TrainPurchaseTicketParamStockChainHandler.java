...
public class TrainPurchaseTicketParamStockChainHandler implements TrainPurchaseTicketChainFilter<PurchaseTicketReqDTO> {
    ...
    public void handler(PurchaseTicketReqDTO requestParam) {
        ...
        String keySuffix = StrUtil.join(StrUtil.UNDERLINE, requestParam.getTrainId(), requestParam.getDeparture(), requestParam.getArrival());
        StringRedisTemplate stringRedisTemplate = (StringRedisTemplate) distributedCache.getInstance();
        List<PurchaseTicketPassengerDetailDTO> passengerDetails = requestParam.getPassengers();
        // 流式操作将乘车人按照座位类型分类，Map Key 为座位类型，Value 为乘车人列表
